...
如图，$Root$为根的子树中存在答案（蓝色实边路径），可以看成以$Root2$为根的两棵子树存在答案，所以只用处理情况2就行了，可以用分治的方法，这~~应该~~是点分治的基本原理
...
这是第二种情况，第一种情况就让距离根为$k$的点跟重心配对就行了，因为重心到重心的距离为0
...
补充：经$Aufun$大爷~~嘲讽~~教导后，因为有的题可以用桶排序，所以复杂度可以降到$O(logn)$
